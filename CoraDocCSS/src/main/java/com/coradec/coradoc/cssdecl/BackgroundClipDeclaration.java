...
/**
 * ​​Implementation of the CSS background-clip declaration.
 */
@SuppressWarnings("ClassHasNoToStringMethod")
public class BackgroundClipDeclaration extends BasicCssDeclaration {

    static BackgroundClip getDefault() {
        return BackgroundClip.BORDER_BOX;
    }

    static boolean isClip(final ParserToken token) {
        if (!(token instanceof Identifier)) return false;
        try {
            BackgroundClip.valueOf(((Identifier)token).getEnumTag());
            return true;
    ...
    /**
     * Initializes a new instance of BackgroundClipDeclaration from the specified declaration.
     *
     * @param source the declaration.
     */
    protected BackgroundClipDeclaration(final ModifiableDeclaration source) {
        super(source);
    }

    /**
     * Initializes a new instance of BackgroundClipDeclaration with the specified clip token.
     *
     * @param clip the clip token.
     * @throws IllegalArgumentException if the specified token is not a valid background clip.
     */
    protected BackgroundClipDeclaration(final ParserToken clip) {
    ...
    protected ProcessingState base(final ParserToken token) {
        if (token instanceof Identifier) try {
            clip = BackgroundClip.valueOf(((Identifier)token).getEnumTag());
        } catch (IllegalArgumentException e) {
    ...
    public BackgroundClip getClip() {
        return clip != null ? clip : getDefault();
    }

    @Override public void apply(final Style style) {
        style.setBackgroundClip(getClip());
    }
}
