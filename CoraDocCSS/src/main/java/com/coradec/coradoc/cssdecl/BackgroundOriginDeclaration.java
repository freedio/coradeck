...
/**
 * ​​Implementation of the CSS background-origin declaration.
 */
@SuppressWarnings("ClassHasNoToStringMethod")
public class BackgroundOriginDeclaration extends BasicCssDeclaration {

    static BackgroundOrigin getDefault() {
        return BackgroundOrigin.PADDING_BOX;
    }

    static boolean isOrigin(final ParserToken token) {
        if (!(token instanceof Identifier)) return false;
        try {
            BackgroundOrigin.valueOf(((Identifier)token).getEnumTag());
            return true;
    ...
    private BackgroundOrigin origin;

    /**
     * Initializes a new instance of BackgroundOriginDeclaration from the specified declaration.
     *
     * @param source the declaration.
     */
    protected BackgroundOriginDeclaration(final ModifiableDeclaration source) {
        super(source);
    }

    /**
     * Initializes a new instance of BackgroundOriginDeclaration with the specified origin token.
     *
     * @param origin the origin token.
     * @throws IllegalArgumentException if the specified token is not a valid background origin.
     */
    protected BackgroundOriginDeclaration(final ParserToken origin) {
    ...
    protected ProcessingState base(final ParserToken token) {
        if (token instanceof Identifier) try {
            origin = BackgroundOrigin.valueOf(((Identifier)token).getEnumTag());
        } catch (IllegalArgumentException e) {
    ...
    public BackgroundOrigin getOrigin() {
        return origin != null ? origin : getDefault();
    }

    @Override public void apply(final Style style) {
        style.setBackgroundOrigin(getOrigin());
    }
