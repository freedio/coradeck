...
/**
 * ​​Implementation of a class selector.
 */
public class ClassSelector extends BasicSelector {
    ...
    @Override public boolean matches(final List<String> path, final ValueMap attributes) {
        return attributes.lookup("class").map(attr -> attr.equals(getName())).orElse(false);
    }
